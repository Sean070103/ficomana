...
export default function PaymentVerificationQueue() {
  ...
  const handleApprove = async (booking: Booking) => {
    ...
    try {
      ...
      await sendPaymentApprovedEmail(updatedBooking)

      alert(`Booking ${booking.id} successfully confirmed! Email notification sent.`)
  ...
  const handleRejectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await sendPaymentRejectedEmail(updatedBooking, finalReason)
  ...
  return (
    <div className="space-y-6 font-sans">
      <div>
        <h1 className="text-xl font-bold uppercase tracking-wider text-slate-800">Payment Verification</h1>
        <p className="text-xs text-slate-500">Review uploaded GCash screenshots and approve or reject sessions.</p>
      </div>

      {bookings.length === 0 ? (
        <div className="bg-white border border-slate-200 p-16 text-center shadow-sm">
          <div className="w-16 h-16 bg-green-50 border border-green-100 flex items-center justify-center text-green-600 rounded-full mx-auto mb-4">
            <Check className="w-8 h-8" />
          </div>
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-800">Verification Queue Empty</h3>
          <p className="text-xs text-slate-400 mt-1">All booking deposits have been verified. Excellent job!</p>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {bookings.map((booking) => (
            <div key={booking.id} className="bg-white border border-slate-200 shadow-sm flex flex-col justify-between overflow-hidden">
              {/* Receipt Preview Thumbnail */}
              <div 
                className="h-48 bg-slate-100 relative overflow-hidden group cursor-pointer border-b border-slate-150"
                onClick={() => {
              ...
              >
                ...
                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-xs font-semibold uppercase tracking-widest">
                  View Large Receipt
                </div>
              </div>
              ...
              <div className="p-5 space-y-4">
                <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{booking.id}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-y-2 text-xs">
                  <div>
                    <p className="text-slate-400 font-medium text-[9px] uppercase tracking-wider">Date & Time</p>
                    <p className="font-semibold text-slate-700">{booking.bookingDate}</p>
                    <p className="text-slate-500 text-[10px] mt-0.5">{booking.bookingTime}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-medium text-[9px] uppercase tracking-wider">GCash Transaction Ref</p>
                    <p className="font-mono font-bold text-slate-700">{booking.transactionRef || 'N/A'}</p>
                  </div>
                  <div className="col-span-2 border-t border-slate-100 pt-3">
                    <div className="flex justify-between items-center text-[11px]">
                      <span className="text-slate-400 font-medium">Deposit Amount:</span>
      ...
      {showReceiptModal && selectedBooking && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white border border-slate-200 shadow-2xl max-w-4xl w-full flex flex-col md:flex-row overflow-hidden max-h-[90vh]">
            {/* Receipt Preview Panel */}
            <div className="md:w-1/2 bg-slate-100 border-r border-slate-200 relative min-h-[300px] flex items-center justify-center">
              {selectedBooking.receiptUrl ? (
                ...
                <div className="text-slate-400">No receipt file uploaded</div>
              )}
            </div>
            ...
            <div className="md:w-1/2 p-8 flex flex-col justify-between overflow-y-auto space-y-6">
              <div className="space-y-6">
                <div className="flex justify-between items-start border-b border-slate-100 pb-4">
                  <div>
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Verification Details</span>
                    <h2 className="text-lg font-bold text-slate-800 mt-1">{selectedBooking.customerName}</h2>
                    <p className="text-xs font-mono text-slate-400 mt-0.5">Reference: {selectedBooking.id}</p>
                  </div>
                  <button 
                    onClick={() => {
                      setShowReceiptModal(false)
                ...
                <div className="grid grid-cols-2 gap-y-4 text-xs">
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">Email</p>
                    <p className="font-semibold text-slate-700">{selectedBooking.customerEmail}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">Phone</p>
                  ...
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">Date & Time</p>
                    <p className="font-semibold text-slate-700">{selectedBooking.bookingDate} at {selectedBooking.bookingTime}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">GCash Transaction Ref</p>
                    <p className="font-mono font-bold text-slate-700 bg-slate-50 border border-slate-200 px-2 py-0.5 inline-block">{selectedBooking.transactionRef || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">Total Package Price</p>
                    <p className="font-bold text-slate-700">₱{selectedBooking.price.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">Required Deposit</p>
                    <p className="font-bold text-green-600">₱{selectedBooking.depositAmount.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider">Remaining Balance</p>
                  ...
                  {selectedBooking.note && (
                    <div className="col-span-2 border-t border-slate-150 pt-3">
                      <p className="text-slate-400 font-medium text-[8px] uppercase tracking-wider mb-1">Pre-shoot Request Note</p>
                      <p className="bg-slate-50 p-3 border border-slate-100 rounded text-slate-600 leading-relaxed italic">
                        "{selectedBooking.note}"
                      </p>
                    </div>
                  )}
      ...
      {showRejectModal && selectedBooking && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-55 animate-fade-in">
          <form 
            onSubmit={handleRejectSubmit} 
            className="bg-white border border-slate-200 shadow-2xl max-w-md w-full p-6 md:p-8 space-y-5"
          >
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-bold text-slate-800">Reject Payment Receipt</h3>
                <p className="text-[10px] text-slate-400 font-mono mt-0.5">Booking ID: {selectedBooking.id}</p>
              </div>
              <button 
                type="button"
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-primary focus:outline-none p-3 text-xs font-semibold"
                >
              ...
              {rejectionReason === 'Other' && (
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-primary focus:outline-none p-3 text-xs font-semibold resize-none"
                  />
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3 border-t border-slate-100 pt-4 mt-2">
              <button
                type="button"
                onClick={() => setShowRejectModal(false)}
                className="flex-1 border border-slate-200 text-slate-700 text-xs font-bold uppercase tracking-wider py-3 hover:bg-slate-50"
              >
                Cancel
              </button>
              <button
